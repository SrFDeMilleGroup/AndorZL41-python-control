...
class scipy_curve_fit_tester:
    def __init__(self, array_size=1000000, repeat=5, tolerance=1e-6, max_num_iter=50):
...
class modified_torch_curve_fit_tester(torchimize_curve_fit_tester):
    ...
    def curve_fit_test(self, print_time=True):
        ...
        for _ in range(self.repeat):
            ...
            # returns a list of tensors consisting of fitted parameters found in each iteration
            fitted_param, i, success = gaussian_1d_fit_lma(self.init_param, self.xdata, self.ydata, 
        ...
        return (fitted_param.cpu().numpy(), self.true_param, self.init_param.cpu().numpy(), 
                self.xdata.cpu().numpy(), self.ydata.cpu().numpy(), 
                self.gaussian_1d(self.xdata.cpu().numpy(), *fitted_param.cpu().numpy()))
