...
class scipy_curve_fit_tester:
    def __init__(self, array_size_x=310, array_size_y=310, repeat=5, tolerance=1e-6, max_num_iter=50):
        ...
        self.max_func_call = self.max_num_iter*2*(self.true_param.shape[0]+1)
...
class torchimize_curve_fit_tester(scipy_curve_fit_tester):
    ...
    def curve_fit_test(self, print_time=True):
        ...
        cost_func = lambda p, x, y, data: (data - self.gaussian_2d_torch(*p)(x, y))**2
        ...
        for _ in range(self.repeat):
            ...
            fitted_param = lsq_lma(init_param, function=cost_func, jac_function=jac_func, 
                                    args=(X_torch.ravel(), Y_torch.ravel(), data_torch.ravel()), max_iter=self.max_num_iter, 
...
torchimize_test = torchimize_curve_fit_tester()
