...
class ImageGUI(Scrollarea):
    ...
    def place_signal_images(self):
        ...
        for i, name in enumerate(self.signal_image_name_list):
            ...
            image_widget.image_roi.sigRegionChanged.connect(self.update_image_roi(source_image_name=name))
    ...
    def place_axis_plots(self):
        ...
        self.x_plot_lr.sigRegionChanged.connect(self.update_image_roi(source_image_name="x_plot"))
        ...
        self.y_plot_lr = pg.LinearRegionItem([self.parent.config.getint("image_control", "software_roi_ymin"),
                                              self.parent.config.getint("image_control", "software_roi_ymax")], 
                                             swapMode="block")
        self.y_plot_lr.setBounds([0, self.parent.config.getint("image_control", "hardware_roi_height")])
        y_plot.addItem(self.y_plot_lr)
        self.y_plot_lr.sigRegionChanged.connect(self.update_image_roi(source_image_name="y_plot"))
        ...
        # x_plot.getAxis("right").setTicks([])
        x_plot.getAxis("right").setStyle(**tickstyle)
        data_roi = self.starting_data[self.parent.config.getint("image_control", "xmin") : self.parent.config.getint("image_control", "xmax"),
                                      self.parent.config.getint("image_control", "ymin") : self.parent.config.getint("image_control", "ymax")]
